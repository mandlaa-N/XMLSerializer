...
//   Program Module: XMLExport Class implementation cpp file
//
...
QString XMLExport::objectToXml(const QObject* doptr,
    ...
    for (int i= 0; i < meta->propertyCount(); ++i) { /* Iterate
        ...
        result += QString (
                ...
                .arg(qvariant_cast<QString>(qv));
    }

    QObjectList childlist = doptr->findChildren<QObject*>(QString());
    foreach (QObject* objptr, childlist) { /* Iterate through
        the child list. */
        if (objptr->parent()==doptr) { /* Iterating through all the object tree items, so we skip over those. */
